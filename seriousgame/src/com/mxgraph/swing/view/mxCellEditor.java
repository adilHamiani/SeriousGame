...
import diaguml.ConstDifficile;
import diaguml.Etiquette;
	...
	public void stopEditing(boolean cancel)
	{
		if (editingCell != null)
		{
			...
			if (!cancel)
			{
				...
				//System.out.println("Nouvelle Valeur = "+getCurrentValue());
				...
				System.out.println("La liste est nulle ? "+(listeLiaison == null));
				if(listeLiaison != null){
					for(int i=0; i<listeLiaison.size(); i++){
						diaguml.Liaison liaisonI = listeLiaison.get(i);
						if(liaisonI.getEtDep().getId().equals(((mxCell) cell).getId())){
							System.out.println("Ancienne valeur dep "+liaisonI.getEtDep().getNom());
							liaisonI.getEtDep().setNom(getCurrentValue());
							System.out.println("Nouvelle valeur dep "+liaisonI.getEtDep().getNom());
						}
						if(liaisonI.getEtArr().getId().equals(((mxCell) cell).getId())){
							System.out.println("Ancienne valeur arr "+liaisonI.getEtArr().getNom());
							liaisonI.getEtArr().setNom(getCurrentValue());
